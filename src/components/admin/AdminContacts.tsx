...
import { Search, Eye, Trash, CheckCircle, XCircle, Clock, AlertCircle } from 'lucide-react';
import { 
  Dialog,
  DialogContent,
  DialogDescription,
...
const AdminContacts = () => {
  ...
  const handleMarkAsRead = (id: string) => {
    ...
    toast({
      ...
      description: `Contact request has been marked as ${newStatus}`,
    });
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <div className="flex gap-2 text-sm text-gray-400">
            <span>{filteredContacts.filter(c => c.status === 'new').length} new messages</span>
            <span>•</span>
            ...
            {filteredContacts.length === 0 ? (
              ...
              filteredContacts.map((contact) => (
                <TableRow key={contact.id} className={contact.status === 'new' ? 'bg-blue-500/5' : ''}>
                  <TableCell>
                    {contact.status === 'new' && <Clock className="h-4 w-4 text-yellow-500" />}
                    {contact.status === 'read' && <CheckCircle className="h-4 w-4 text-blue-500" />}
                    {contact.status === 'responded' && <AlertCircle className="h-4 w-4 text-green-500" />}
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        className="h-8 px-2 border-gray-700"
                      >
                        {contact.status === 'new' ? <CheckCircle className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription className="text-gray-400 text-sm pt-2">
                Received on {selectedContact.createdAt?.toDate ? selectedContact.createdAt.toDate().toLocaleDateString() : 'N/A'}
              </DialogDescription>
            </DialogHeader>
